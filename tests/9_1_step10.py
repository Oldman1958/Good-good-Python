...
# Создаём генератор, который циклически перебирает список городов миллион раз
city_generator = (cities[i % len(cities)] for i in range(1_000_000))


# Получаем первые 20 элементов генератора и выводим их в одну строку через пробел
print(' '.join(next(city_generator) for _ in range(20)))
