...
и возвратить список из соответствующих значений ключей словаря. Сам словарь d при этом должен оставаться неизменным.
...
def get_sort(d):
    inner_d = dict(sorted(d.items(), reverse=True))
    return list(inner_d.values())