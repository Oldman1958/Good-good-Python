"""
Подвиг 5.
Продолжите программу, в которой уже реализовано чтение данных (чисел) из входного потока
и сохранение их в виде кортежа t:

t = tuple(map(int, input().split()))  # кортеж из целых чисел
s = 0  # начальное значение суммы элементов

Используя моржовый оператор и переменную s, необходимо с помощью генератора списка сформировать новый список lst
из сумм текущего и всех предыдущих значений кортежа t. Например, для кортежа:
